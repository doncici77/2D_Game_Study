...
### 13일차
- 딜레이 시간 줄이기, 클리어 맵에 ui 없애기, 로프 세팅 수정
- 포탈을 막지 말고 처음으로 이동
- 몬스터 두번 죽는 버그
  - 적의 공격 판정 콜라이더를 적 오브젝트랑 분리함
- 몹죽을때 피가 살짝 남는 ui적인 부분 수정, 적이 벽 뚫고가는거
  - 추적을 플레이어와 적과의 y좌표 차이가 일정값이상 커지면 추적 중지로 해결함
- 그래플링의 거리 원으로 게임화면에 표현 추가
- 애니메이션 관련으로 문제 스크립트 수정
- 배경음 초기값 조정, 옵션 창 수정, 옵션 X 버튼 배경변경
- 로프할때 플레이어한테 임펙트, 로프가 닿이는 부분에도 임펙트, 로프 소리 수정, 뛸때 발 부분 작은 임펙트
- 원거리 무기 변경시 소리 추가, 마지막 다듬기
